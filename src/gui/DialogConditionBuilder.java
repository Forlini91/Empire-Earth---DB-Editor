...
public class DialogConditionBuilder extends JDialog {
	...
	private static GridBagConstraints gbc_lblName = new GridBagConstraints(),
			...
			gbc_textSearchField = new GridBagConstraints(),
			gbc_checkSearchField = new GridBagConstraints(),
			gbc_comboBoxEnumSearchField = new GridBagConstraints(),
			gbc_comboBoxEntrySearchField = new GridBagConstraints(),
...

	{
		...
		contentPanel.add(textField, gbc_textSearchField);
		contentPanel.add(checkField, gbc_checkSearchField);
		contentPanel.add(comboBoxEnum, gbc_comboBoxEnumSearchField);
		contentPanel.add(comboBoxEntry, gbc_comboBoxEntrySearchField);
	...
	private DialogConditionBuilder(Window parent, ConditionOperator condition) {
		...
		final FieldType type = condition.datFile.datStructure.getFieldStruct(condition.index).type;
		switch (type) {
			case BOOLEAN:
	...
	private void setDatFile(DatFile datFile) {
		...
		final int length = datStructure.fieldStructs.length;
		final boolean extraField = datStructure.extraField != null;
		final String[] fields = new String[length + (extraField ? 2 : 1)];
		fields[0] = "<Any field>";
	...
	private void comboBoxCheckSelection(DatStructure datStructure) {
		final int index = comboBoxField.getSelectedIndex() - 1;
		try {
			final FieldStruct field = datStructure.getFieldStruct(index);
			if (field == null) {
				if (searchMode != null) {
					searchMode = null;
			...
			Util.printException(this, exc, "An error occurred while selecting the field", "Error", true);
		}
	}
	...
	private void buttonOkClick(DatFile datFile, DatStructure datStructure) {
		...
		try {
			...
			Util.printException(this, exc, "An error occurred while checking the field", "Error", true);
			return;
		}

		final Operator operator = (Operator) comboBoxOperator.getSelectedItem();
		Object value;
		if (type == null) {
			value = text.trim();
		...
		try {
			...
			Util.printException(this, e, "An error occurred while saving the condition", "Error", true);
		}
		setVisible(false);
	}
	...
	public static ConditionOperator buildCondition(Window parent, DatFile datFile) {
		final DialogConditionBuilder builder = new DialogConditionBuilder(parent, datFile);
	...
	public static void buildCondition(Window parent, ConditionOperator condition) {
		final DialogConditionBuilder builder = new DialogConditionBuilder(parent, condition);
		builder.setVisible(true);
	}
	...
	static {
		...
		gbc_textSearchField.fill = GridBagConstraints.BOTH;
		gbc_textSearchField.gridx = 1;
		gbc_textSearchField.gridy = 3;
		gbc_textSearchField.insets = new Insets(0, 5, 5, 5);

		gbc_checkSearchField.fill = GridBagConstraints.BOTH;
		gbc_checkSearchField.gridx = 1;
		gbc_checkSearchField.gridy = 3;
		gbc_checkSearchField.insets = new Insets(0, 5, 5, 5);

		gbc_comboBoxEnumSearchField.fill = GridBagConstraints.BOTH;
		gbc_comboBoxEnumSearchField.gridx = 1;
		gbc_comboBoxEnumSearchField.gridy = 3;
		gbc_comboBoxEnumSearchField.insets = new Insets(0, 5, 5, 5);

		gbc_comboBoxEntrySearchField.fill = GridBagConstraints.BOTH;
		gbc_comboBoxEntrySearchField.gridx = 1;
		gbc_comboBoxEntrySearchField.gridy = 3;
		gbc_comboBoxEntrySearchField.insets = new Insets(0, 5, 5, 5);
