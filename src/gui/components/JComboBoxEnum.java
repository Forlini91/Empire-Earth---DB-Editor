...
public class JComboBoxEnum extends JComboBox<EnumValue> implements EntryFieldInterface, ItemListener, MouseListener, KeyListener {
	...
	private final ListSearcher<EnumValue> searcher = new ListSearcher<>(NAME_MATCHER, ID_MATCHER);
		...
		if (isEnabled()) {
			SwingUtilities.invokeLater(() -> {
				...
				if (text == null || text.isEmpty()) {
					...
					final List<EnumValue> results = searcher.find(fieldStruct.enumValues, null, text);
					if (results != null) {
						final EnumValue enumValue = searcher.findNext();
						if (enumValue != null) {
							final ComboPopup popup = (ComboPopup) getUI().getAccessibleChild(this, 0);
