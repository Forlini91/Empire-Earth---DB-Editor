...
public class JComboBoxEnumEffect extends JComboBox<EffectCode> implements EntryFieldInterface, ItemListener, MouseListener, KeyListener {
	...
	private final ListSearcher<EffectCode> searcher = new ListSearcher<>(NAME_MATCHER, ID_MATCHER);
	...
	public void updateEditor() {
		final boolean[] toEnable = { true, true, false, false, false, false, false, false, true, false, false, false, false, false };
		...
		if (isEnabled()) {
			SwingUtilities.invokeLater(() -> {
				...
				if (text == null || text.isEmpty()) {
					...
					final List<EffectCode> results = searcher.find(EffectCode.values(), null, text);
					if (results != null) {
						final EffectCode result = searcher.findNext();
						if (result != null) {
							final ComboPopup popup = (ComboPopup) getUI().getAccessibleChild(this, 0);
